<?php
/**
 * Contenu importé de la maquette validée : massages.html.
 *
 * @package BalneoV2
 */

?>
<main id="contenu-principal">
<section class="hero-page">
    <div class="hero-page__media media-plein">
      <img src="<?php echo esc_url( get_theme_file_uri( '/assets/photos/balneo-73.webp' ) ); ?>" alt="" width="2560" height="1707" loading="eager" fetchpriority="high" decoding="async" srcset="<?php echo esc_attr( balneo_v2_theme_image_srcset( 'balneo-73.webp' ) ); ?>" sizes="100vw">
    </div>
    <div class="hero-page__voile"></div>
    <div class="hero-page__contenu">
      <p class="hero-page__libelle">Soins bien-être</p>
      <h1 class="hero-page__titre">Massages</h1>
      <span class="filet-titre" aria-hidden="true"></span>
    </div>
  </section>
<section class="section-contenu section-contenu--claire">
  <div class="conteneur bloc-contenu apparition">
    <p>Accordez-vous une parenthèse hors du temps, où chaque geste est pensé pour vous offrir une profonde relaxation, une harmonie intérieure et un véritable moment de lâcher-prise.</p>
    <div class="encadre-info">
      <p class="encadre-info__titre">Bon à savoir</p>
      <ul>
        <li>Les massages se font <strong>sur rendez-vous</strong>, par téléphone au <a href="tel:+33468756050">04 68 75 60 50</a></li>
        <li>La durée indiquée comprend l'installation, l'entretien personnel et le massage</li>
        <li>Merci d'arriver 20 min avant votre massage afin de prendre une douche</li>
      </ul>
    </div>
    <h2>La carte des massages bien-être</h2>
    <div class="liste-services">
      <div class="element-service"><div><p class="element-service__nom">Détente anti-stress</p><p class="element-service__duree">30, 60 ou 90 min</p></div></div>
      <p class="element-service__description">Massage profondément relaxant associant gestes lents, fluides et enveloppants à des manœuvres plus profondes.</p>
      <div class="element-service"><div><p class="element-service__nom">Tonique et décontractant</p><p class="element-service__duree">30 ou 60 min</p></div></div>
      <p class="element-service__description">Dynamisant et revitalisant, idéal pour les sportifs et la récupération musculaire.</p>
      <div class="element-service"><div><p class="element-service__nom">Tête — Mains — Pieds</p><p class="element-service__duree">30 ou 60 min</p></div></div>
      <div class="element-service"><div><p class="element-service__nom">Harmonisation des énergies</p><p class="element-service__duree">60 ou 90 min</p></div></div>
      <div class="element-service"><div><p class="element-service__nom">Massage femme enceinte</p><p class="element-service__duree">30 ou 60 min — 4e au 8e mois</p></div></div>
      <div class="element-service"><div><p class="element-service__nom">Massage douceur enfant</p><p class="element-service__duree">5 à 14 ans — accompagné en cabine</p></div></div>
      <div class="element-service"><div><p class="element-service__nom">Rituel Océan Relax</p><p class="element-service__duree">60 ou 90 min — à partir de 15 ans</p></div></div>
      <p class="element-service__description">30 min d'hydromassage + massage manuel personnalisé de 30 ou 60 min.</p>
    </div>
    <h2>Tarifs</h2>
    <h3>Massages bien-être avec 2h30 de balnéo</h3>
    <div class="tableau-adaptatif"><table>
      <tr><th>Formule</th><th>Tarif</th></tr>
      <tr><td>30 min + 2h30 balnéo</td><td>66 €</td></tr>
      <tr><td>60 min + 2h30 balnéo</td><td>83 €</td></tr>
      <tr><td>90 min + 2h30 balnéo</td><td>136 €</td></tr>
      <tr><td>Carte 6 massages 1h + 2h30 balnéo</td><td>450 €</td></tr>
    </table></div>
    <h3>Massages bien-être avec balnéo illimitée</h3>
    <div class="tableau-adaptatif"><table>
      <tr><th>Formule</th><th>Tarif</th></tr>
      <tr><td>30 min + balnéo illimitée</td><td>73 €</td></tr>
      <tr><td>60 min + balnéo illimitée</td><td>90 €</td></tr>
      <tr><td>90 min + balnéo illimitée</td><td>143 €</td></tr>
    </table></div>
    <p>Les massages peuvent être réalisés en duo dans notre cabine double (2 prestations à acheter). Le Rituel Océan Relax s'effectue en cabine simple uniquement.</p>
    <p><a href="<?php echo esc_url( home_url( '/hydromassages/' ) ); ?>">Voir aussi : Hydromassages</a> · <a href="<?php echo esc_url( home_url( '/recuperation/' ) ); ?>">Composer sa récupération</a></p>
  </div>
</section>
<div class="bande-appel">
    <p>Réservez par téléphone au 04 68 75 60 50</p>
    <a href="https://gruissan-balneo.horanet.com/" class="bouton bouton--plein" target="_blank" rel="noopener">Billetterie &amp; cadeaux<i class="fa-solid fa-ticket" aria-hidden="true"></i></a>
    <p style="margin-top:0.75rem;font-size:0.75rem;opacity:0.7">Les e-billets ne sont pas nominatifs</p>
  </div>
</main>
