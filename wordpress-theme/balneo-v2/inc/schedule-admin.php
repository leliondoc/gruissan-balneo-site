<?php
/**
 * Administration du planning, réservée aux personnes autorisées à éditer les pages.
 *
 * @package BalneoV2
 */
...
/** Ajoute le menu réservé aux éditrices. */
...
/**
 * Charge les ressources uniquement sur l'écran du planning.
 *
 * @param string $hook Identifiant de l'écran WordPress.
 */
function balneo_v2_schedule_admin_assets( string $hook ): void {
	if ( 'toplevel_page_balneo-planning' !== $hook || ! current_user_can( 'edit_pages' ) ) {
		return;
	}
...
/**
 * Construit une URL interne de l'administration du planning.
 *
 * @param array $args Paramètres de navigation.
 * @return string
 */
...
function balneo_v2_schedule_admin_save(): void {
	...
	if ( ! isset( $_POST['form_complete'] ) || '1' !== $_POST['form_complete'] ) {
		...
		$result = balneo_v2_schedule_save( $input, $revision );
	}
	$id   = $result['entry']['id'] ? $result['entry']['id'] : 'new';
	$args = array( 'edit' => $id );
	if ( $result['errors'] ) {
		$token = wp_generate_uuid4();
		set_transient(
			'balneo_schedule_feedback_' . get_current_user_id() . '_' . $token,
			array(
				'entry'    => $result['entry'],
...
/**
 * Affiche une liste déroulante étiquetée.
 *
 * @param string $name    Nom du champ.
 * @param string $label   Libellé visible.
 * @param string $value   Valeur sélectionnée.
 * @param array  $options Choix proposés.
 */
function balneo_v2_schedule_select( string $name, string $label, string $value, array $options ): void {
	...
	foreach ( $options as $key => $text ) :
		?>
		<option value="<?php echo esc_attr( $key ); ?>" <?php selected( $value, (string) $key ); ?>><?php echo esc_html( $text ); ?></option>
		<?php
endforeach;
	?>
	</select></div>
...
/**
 * Affiche les champs d'un état du planning.
 *
 * @param string $prefix Préfixe des noms de champs.
 * @param array  $state  Informations enregistrées.
 */
function balneo_v2_schedule_state_fields( string $prefix, array $state ): void {
	...
	<?php
	balneo_v2_schedule_select(
		$prefix . '[mode]',
		'Affichage',
		$mode,
		array(
			'info'   => 'Afficher les informations',
			'closed' => 'Afficher une fermeture / annulation',
			'hidden' => 'Ne pas afficher la carte',
		)
	);
...
/**
 * Affiche les cases à cocher d'une récurrence.
 *
 * @param string $name    Nom des champs.
 * @param string $label   Légende du groupe.
 * @param array  $options Choix possibles.
 * @param array  $values  Choix enregistrés.
 */
function balneo_v2_schedule_checks( string $name, string $label, array $options, array $values ): void {
	...
	<?php endforeach; ?></div><p class="description">Aucune case cochée = tous.</p></fieldset>
	<?php
}

/**
 * Affiche une règle éditable.
 *
 * @param string $index         Identifiant de la règle dans le formulaire.
 * @param array  $rule          Règle enregistrée.
 * @param array  $default_state État par défaut de l'activité.
 */
function balneo_v2_schedule_rule_fields( string $index, array $rule, array $default_state ): void {
	$prefix = 'entry[rules][' . $index . ']';
	$kind   = $rule['kind'] ?? ( isset( $rule['dates'] ) ? 'exception' : 'period' );
	$state  = array_merge( $default_state, $rule );
	?>
	...
	balneo_v2_schedule_select(
		...
		$kind,
		array(
			'period'    => 'Période / récurrence',
			'exception' => 'Exception à une date précise',
		)
	);
	?>
	...
	</div><?php balneo_v2_schedule_state_fields( $prefix, $state ); ?></fieldset>
	<?php
}

/** Affiche la liste ou la fiche, sans modifier le planning. */
function balneo_v2_schedule_admin_page(): void {
	...
	if ( isset( $_GET['feedback'] ) && is_string( $_GET['feedback'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Message privé à l'utilisateur connecté.
		$key      = 'balneo_schedule_feedback_' . get_current_user_id() . '_' . sanitize_key( wp_unslash( $_GET['feedback'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Lecture d'un message propre à l'utilisateur connecté.
		$feedback = get_transient( $key );
		delete_transient( $key );
	}
	foreach ( $store['entries'] as $candidate ) {
	...
	if ( is_array( $feedback ) ) :
		?>
		<div class="notice notice-error"><p><strong>La fiche n’a pas été enregistrée.</strong></p><ul>
		<?php
...
endforeach;
		...
		<?php
	elseif ( isset( $_GET['saved'] ) ) : // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Notification en lecture seule.
		?>
		<div class="notice notice-success is-dismissible"><p>Activité enregistrée. Le calendrier utilise désormais ces informations.</p></div>
<?php endif; ?>
	<?php
	if ( '' !== $edit && null === $entry ) :
		?>
		<div class="notice notice-error"><p>Activité introuvable.</p></div>
<?php endif; ?>
	<?php if ( null === $entry ) : ?>
		...
		<?php foreach ( $sorted as $item ) : ?>
			<?php $status = balneo_v2_schedule_status( $item, balneo_v2_schedule_today() ); ?>
			<tr><th scope="row"><?php echo esc_html( $item['title'] ); ?></th><td><?php echo esc_html( $themes[ $item['theme'] ]['label'] ?? $item['theme'] ); ?></td>
			<td><strong><?php echo esc_html( $status['label'] ); ?></strong>
			<?php
			if ( $status['next'] ) :
				?>
				<br><?php echo esc_html__( 'Prochaine séance :', 'balneo-v2' ); ?> <?php echo esc_html( implode( '/', array_reverse( explode( '-', $status['next'] ) ) ) ); ?>
				<?php
endif;
			?>
			<?php
			if ( $status['until'] ) :
				?>
				<br><?php echo esc_html__( 'Programmation jusqu’au :', 'balneo-v2' ); ?> <?php echo esc_html( implode( '/', array_reverse( explode( '-', $status['until'] ) ) ) ); ?>
				<?php
endif;
			?>
			</td><td><?php echo esc_html( (string) count( $item['rules'] ) ); ?></td><td><?php echo esc_html( (string) $item['position'] ); ?></td><td><a class="button" aria-label="<?php echo esc_attr( 'Modifier ' . $item['title'] ); ?>" href="<?php echo esc_url( balneo_v2_schedule_admin_url( array( 'edit' => $item['id'] ) ) ); ?>">Modifier</a></td></tr>
		<?php endforeach; ?>
		<?php
		if ( ! $sorted ) :
			?>
			<tr><td colspan="6">Aucune activité enregistrée. Ajoutez votre première carte.</td></tr>
<?php endif; ?>
	...
	<?php else : ?>
		...
		</div><label class="planning-enabled"><input type="checkbox" name="entry[enabled]" value="1" <?php checked( ! empty( $entry['enabled'] ) ); ?>> Activer cette activité (affichage selon les périodes et exceptions)</label></section>
		...
		<div data-planning-rules>
		<?php
		foreach ( $entry['rules'] as $index => $rule ) {
			balneo_v2_schedule_rule_fields( (string) $index, $rule, $entry['default'] );
		}
		?>
		</div>
		...
		<?php
		balneo_v2_schedule_rule_fields(
			'__INDEX__',
			array( 'kind' => 'period' ),
			array(
				'time'   => '',
		...
		<div class="planning-actions"><button type="submit" class="button button-primary button-hero">Enregistrer l’activité</button><a class="button" href="<?php echo esc_url( balneo_v2_schedule_admin_url( array( 'edit' => $edit ? $edit : 'new' ) ) ); ?>">Recharger la fiche enregistrée</a></div>
