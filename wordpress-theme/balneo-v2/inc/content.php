...
function balneo_v2_seed_pages(): void {
	...
	foreach ( $seeds as $slug => $seed ) {
		...
		if ( $page ) {
			update_post_meta( $page->ID, '_balneo_v2_seed_source', 'maquette-1.0' );
			if ( 'accueil' === $slug ) {
				$home_id = (int) $page->ID;
			}
		}
	...
	update_option( 'balneo_v2_content_schema_version', '1.0.0' );
}
...
function balneo_v2_maybe_migrate_content(): void {
	if ( '1.0.0' === get_option( 'balneo_v2_content_schema_version' ) ) {
		return;
	}
